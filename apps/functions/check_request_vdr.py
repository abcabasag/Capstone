...
layout = html.Div(
    [
        dbc.Button(html.Img(src='/assets/back.svg'),
                   id='back_button',
                   href='/check_request_home',  # Adjust this URL to match your home page URL
                   outline=True,
                   color="light"
                   ),
        html.H2('Vehicle Dispatch Request Status Viewer'), # Page Header
        html.Hr(),
        dbc.Card( # Card Container
            [
                dbc.CardHeader( # Define Card Header
                    [
                        html.H3('Find Vehicle Dispatch Requests')
                    ]
                ),
                                        ...
                                        dbc.Row(
                                            [
                                                dbc.Col(
                                                    dbc.Input(
                                                        type='text',
                                                        id='VDR_checkrequestfilter',
                                                        placeholder='Type your Request Number here.'
                                                    ),
                                                    width=5
                                                ),
                                                dbc.Col(
                                                    dbc.Button("Search", id="vdrsearch_button", color="primary"),
                                                    width=1,
                                                ),
                                            ],
                                            className = 'mb-3'
...
@app.callback(
    Output('VDR_checkrequestlist', 'children'),
    [Input('vdrsearch_button', 'n_clicks')],
    [State('VDR_checkrequestfilter', 'value')],
)
